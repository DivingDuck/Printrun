...
rem              # Update 2025-12-06: this seems to be solved now with pyinstaller v6.17.0 (2025-11-24).
rem                See https://pyinstaller.org/en/stable/CHANGES.html#bugfix
rem for /F "skip=2 delims= " %%i in ('pip list --outdated --exclude pyglet --exclude pefile') do py -m pip install --upgrade %%i
for /F "skip=2 delims= " %%i in ('pip list --outdated --exclude pyglet') do py -m pip install --upgrade %%i
